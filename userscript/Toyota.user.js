...
(function()
{
    ...
    if (!window.location.hash.includes("p5toyota~toyota_parts"))
    {
        return;
    }
    ...
    const logoDiv = Object.assign(document.createElement("div"), {
        textContent: "PL24 Helper - Toyota",
    ...
    if (document.querySelector("#content"))
    {
        ...
        const observeLoadAnimation = () =>
        {
            const loadAnimation = document.querySelector("[class*=p5_load_animation]");
            if (!loadAnimation) return;

            const intervalId = setInterval(() =>
            {
                if (document.querySelector("[id*='_c0']:not([id*=vinfoBasic]):not([id*=prNr]):not([id*=searchresult]):not([id*=vinfoEquipment]):not([id*=vinfoVDP]) > *"))
                {
